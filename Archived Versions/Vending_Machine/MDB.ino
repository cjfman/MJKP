...
const uint8_t NONE = 3;
const uint8_t RESET = 4;
const uint8_t WAIT = 5;
const uint8_t SETUP = 6;
const uint8_t POLL = 7;
const uint8_t TUBE_STATUS = 8;
const uint8_t COIN_TYPE = 9;
const uint8_t DISPENSE = 10;
const uint8_t STACKER = 11;
const uint8_t BILL_TYPE = 12;
...
//    }
  ...
  if (changer_state == TUBE_STATUS)
  {
    this->changerTubeStatus();
    return;
  }
  if (changer_state == COIN_TYPE)
  {
    long end = this->read();
    if (end == -1)
    {
...
//  Serial.println(dispense, DEC);
  if (queue != 16)
  {
//    Serial.println("Checking if dispensed...");
    uint8_t end;
    end = this->read();
    if (end == -1)
    {
...
//      Serial.println(coin_value[queue], DEC);
...
//      Serial.println(buffer[0], HEX);
  ...
  if (dispense >= 5 && queue == 16) //Nickel Dispenser Broken. Give Dimes Instead
  {
...
//    }
  ...
  if (accepted >= 0)
  {
    ...
    if (routing == 0x00) //Bill Stacked
    {
      ...
      if (funds_enable)
      {
        if (last_stack + 500 < current && !no_add)
        {
